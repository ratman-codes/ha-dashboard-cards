/* flat-climate-card v2.4.1 - custom Lovelace card for the main dashboard.
   ...
   - v2.3 (2026-09-07): AC / WINDOW STRIPS ON THE 14d+ TABS. The 24h/7d tabs
     ...
     temperature chart on 14d/1m/3m/6m/1y: row 1 = AC (blue; heating in
     heat-orange), row 2 = windows open (green); cell intensity = the share
     of that hour (14d/1m) or day (3m+) the signal was on, read from the
     hourly/daily long-term statistics of 0/1 signal sensors that never
     purge: cooling_stats (default sensor.nest_cooling_signal),
     heating_stats (default sensor.nest_heating_signal), window_stats
     (default none - a 0/1 template sensor mirroring the window contacts;
     the row is simply absent until one is configured). Any false/missing/
     failed source = that row absent, never an empty guess. Owner picked
     the strips (option B) over full-height columns: at these scales the
     question is "which days / which part of the day", and columns turned
     into a wash on the phone. One extra statistics fetch per tab, cached.
...
class FlatClimateCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._coolStats = sig(config.cooling_stats, DEF_COOL_STATS);
    this._heatStats = sig(config.heating_stats, DEF_HEAT_STATS);
    this._winStats = sig(config.window_stats, null);
  ...
  _createDom() {
    ...
    root.innerHTML = `
      <style>
        ...
        .pstrips { margin-top: 4px; display: none; }
        .pstrips svg { display: block; width: 100%; height: 16px; }
    ...
    el.innerHTML = `
      <div class="popin">
        ...
        <div class="psect">
          ...
          <div class="pstrips" id="pstrips"></div>
        </div>
        <div class="psect">
          <div class="pshead">
  ...
  /* v2.3: 0/1 signal sensors' long-term statistics for the strips (14d+).
     hour period on 14d/1m, day period on 3m/6m/1y; cached 5 min per tab.
     A missing/failed source yields no rows -> its strip row is absent. */
  async _popSignals(key) {
    const c = this._pop.cache['sig' + key];
    if (c && Date.now() - c.at < REFRESH_MS) return c;
    const r = POP_R[key];
    const ids = [this._coolStats, this._heatStats, this._winStats].filter(Boolean);
    const out = { at: Date.now(), period: r.days <= 31 ? 'hour' : 'day', rows: {} };  // hour on 14d/1m, day on 3m+
    if (!ids.length) { this._pop.cache['sig' + key] = out; return out; }
    const end = new Date(), start = new Date(end.getTime() - r.days * 864e5);
    try {
      const res = await this._hass.callWS({
        type: 'recorder/statistics_during_period',
        start_time: start.toISOString(), end_time: end.toISOString(),
        statistic_ids: ids, period: out.period, types: ['mean'],
      });
      ids.forEach(id => {
        const rows = ((res && res[id]) || []).map(row => {
          let t = row.start;
          if (typeof t === 'string') t = Date.parse(t);
          return { t: t, f: row.mean };
        }).filter(p => p.t && p.f != null && p.f > 0);
        if (rows.length) out.rows[id] = rows;
      });
    } catch (e) { /* strips optional */ }
    this._pop.cache['sig' + key] = out;
    return out;
  }
  _popStripsDom(host, sigs, t0, tEnd) {
    const shown = { ac: false, heat: false, win: false };
    if (!host) return shown;
    if (!sigs || !Object.keys(sigs.rows).length) { host.style.display = 'none'; host.innerHTML = ''; return shown; }
    const w = host.clientWidth || 820, PL = 8;
    const X = x => PL + x * (w - 2 * PL);
    const bMs = sigs.period === 'hour' ? 3600e3 : 864e5;
    const cw = Math.max(0.5, X((t0 + bMs - t0) / (tEnd - t0)) - X(0));
    const cells = (id, y, color, alpha) => {
      const rows = sigs.rows[id];
      if (!rows) return '';
      let s = '';
      rows.forEach(p => {
        const x = X((p.t - t0) / (tEnd - t0));
        if (x < PL - cw || x > w - PL) return;
        s += '<rect x="' + x.toFixed(2) + '" y="' + y + '" width="' + cw.toFixed(2) +
          '" height="6" fill="' + color + '" opacity="' + alpha(p.f).toFixed(2) + '"></rect>';
      });
      return s;
    };
    const a1 = f => 0.25 + 0.75 * Math.min(1, f);
    let svg = '';
    const ac = this._coolStats && cells(this._coolStats, 0, STRIP_COOL, a1);
    const heat = this._heatStats && cells(this._heatStats, 0, STRIP_HEAT, a1);
    const win = this._winStats && cells(this._winStats, 10, STRIP_WIN, f => 0.3 + 0.6 * Math.min(1, f));
    shown.ac = !!ac; shown.heat = !!heat; shown.win = !!win;
    svg += (ac || '') + (heat || '') + (win || '');
    if (!svg) { host.style.display = 'none'; host.innerHTML = ''; return shown; }
    host.innerHTML = '<svg viewBox="0 0 ' + w + ' 16" preserveAspectRatio="none">' + svg + '</svg>';
  ...
  async _popRender() {
    ...
    try {
      ...
      if (seq !== pop.seq) return;
      const sigs = (is24 || key === '7d') ? null : await this._popSignals(key);
      if (seq !== pop.seq) return;
      ...
      if (is24) {
        ...
        if (!t0 || !t1) {   // v2.1.2: say so; _fetchHistory re-renders when history lands
          ...
          this._popStripsDom(el.querySelector('#pstrips'), null);
          return;
        }
      ...
      el.querySelector('#pleg1').innerHTML =
        ...
        ((key === '24h' || key === '7d') ?
          ...
          ' <span style="color:' + STRIP_COOL + '">\u258e</span> AC' :
         (stripRows.ac || stripRows.win) ?
          ' \u00b7 strips:' +
          (stripRows.ac ? ' <span style="color:' + STRIP_COOL + '">\u2582</span> AC' : '') +
          (stripRows.heat ? ' <span style="color:' + STRIP_HEAT + '">\u2582</span> heat' : '') +
          (stripRows.win ? ' <span style="color:' + STRIP_WIN + '">\u2582</span> window' : '') : '');
