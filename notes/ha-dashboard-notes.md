...
## Weather — flat-weather-card v1.3 (v1.1 2026-07-10; v1.2 dew line 2026-07-17; v1.3 chip delta 2026-07-19)
...
## Scoreboard — flat-scoreboard-card v1.0 (2026-07-19)
Leaderboard card for the long-running forecast-accuracy experiment (six providers scored nightly against the local PWS's actual high). Source: `flat-scoreboard-card.js` in this repo — location-clean; all entity ids live in card YAML.
- Anatomy: competition-ranked rows (amber/silver/bronze medal chips), thin avg-error bars on a fixed readable scale (default 0-4F) with the leader in house amber and off-scale entries overflowing with a fade; per-row "today" call (orange once the live actual has busted it by the configured margin) and "yday" miss (orange at blowup threshold); live actual-so-far chip in the header; long-term trend strip for the top N drawn from permanent statistics via the recorder/statistics_during_period websocket call; press feedback + row -> more-info per house style.
- Data durability pattern: the experiment's avg-error template sensors carry `state_class: measurement` so daily aggregates persist forever; day-by-day misses are reconstructible from that LTS plus the day counters. Raw helpers live in a collapsed house-style expander on the same dashboard, safe from stray edits.
