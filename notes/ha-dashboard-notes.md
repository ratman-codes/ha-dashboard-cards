...
## Card Manager — card-manager-card v1.1 (built 2026-07-18)
...
- **Notes:** PIN is a casual-taps deterrent, not security (it lives in dashboard YAML, admin-visible; HA has no per-dashboard password — the dashboard itself is admin-only). Frontend caches data: URL modules, so every write ends with a hard-refresh prompt. v1.0 initial → v1.1 PIN moved from whole-card to Update-mode-only (gate prompted on tab tap, cancelable). Verified with a 20-check mock-hass harness (headless Chromium) incl. wrong-card block with zero WS traffic, backup-before-write, byte-level post-write verify, and self-update guardrails.
